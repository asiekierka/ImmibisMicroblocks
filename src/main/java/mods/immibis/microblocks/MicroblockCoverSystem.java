...
public class MicroblockCoverSystem implements IMicroblockCoverSystem, IPartContainer2 {
	...
	public void readFromNBT(NBTTagCompound tag) {
		parts.clear();
		NBTTagList l = tag.getTagList("ICMP", 10);
		if(l == null)
			return;
		for(int k = 0; k < l.tagCount(); k++) {
			Part p = Part.readFromNBT(l.getCompoundTagAt(k));
			if(p != null)
				parts.add(p);
		}
	}
		...
		for(Part p : parts)
		{
			...
			if(rt != null)
			{
				...
				if(rtdist < best)
				{
					...
					best = rtdist;
					hit = p;
					subHit = SubhitValues.getFromCSPartIndex(k);
				}
			}
			++k;
		}
	...
	// Returns true if the strip is not on one of the same axes as the cover.
	private static boolean canAlwaysPlaceStripWithCover(EnumPosition strip, EnumPosition cover)
	{
		switch(cover)
		{
		case FaceNX: case FacePX:
			return strip.x == EnumAxisPosition.Span;
		case FaceNY: case FacePY:
			return strip.y == EnumAxisPosition.Span;
		case FaceNZ: case FacePZ:
			return strip.z == EnumAxisPosition.Span;
		default:
			return false;
		}
	}
	...
	public boolean canPlace(PartType<?> type, EnumPosition pos) {
		for(Part p : parts)
		{
			...
			if(p.pos.clazz == pos.clazz)
				continue;
			if(p.pos.clazz == EnumPositionClass.Face && pos.clazz == EnumPositionClass.Edge && canAlwaysPlaceStripWithCover(pos, p.pos))
				continue;
			if(pos.clazz == EnumPositionClass.Face && p.pos.clazz == EnumPositionClass.Edge && canAlwaysPlaceStripWithCover(p.pos, pos))
				continue;
	...
	public void writeDescriptionPacket(DataOutput data) throws IOException {
		data.writeShort(wrappedBlock == null ? 0 : Block.getIdFromBlock(wrappedBlock));
	...
	public void readDescriptionPacket(DataInput data) throws IOException {
		...
		wrappedBlock = (wrappedID == 0 ? null : (BlockMultipartBase)Block.getBlockById(wrappedID));
	...
	public List<ItemStack> removePartByPlayer(EntityPlayer ply, int part) {
		...
		if(parts.size() == 0 && te.getClass() == TileMicroblockContainer.class)
			te.getWorldObj().setBlock(te.xCoord, te.yCoord, te.zCoord, Blocks.air, 0, 3);
		else if(parts.size() == 0 && te instanceof TileDummy)
			((TileDummy)te).removeTile();
		else {
	...
	public void convertToContainerBlock() {
		...
		{
			te.getWorldObj().setBlock(te.xCoord, te.yCoord, te.zCoord, MicroblockSystem.microblockContainerBlock, 0, 2);
			IMicroblockCoverSystem c2 = ((TileMicroblockContainer)te.getWorldObj().getTileEntity(te.xCoord, te.yCoord, te.zCoord)).getCoverSystem();
			for(Part p : parts)
				c2.addPart(p);
		}
	}
	...
	public boolean addPartDestroyEffects(int part, EffectRenderer er) {
		...
		parts.get(part).addPartDestroyEffects(te2, er);
	...
	public boolean addPartHitEffects(int part, int sideHit, EffectRenderer er) {
		...
		parts.get(part).addPartHitEffects(te2, sideHit, er);
